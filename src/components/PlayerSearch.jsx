...
export function PlayerSearch({ short_name, overall, player_id, club_team_id, nation_team_id}){
    ...
    return (
        <Link to={`/player/${short_name}`}>
            <div onClick={() => {
            onSearch(short_name)
            }} className="flex flex-row relative bg-gray-50 items-center w-96 justify-between py-1 border-b border-gray-300 text-custom-maroon hover:bg-custom-maroon hover:text-white">
                ...
                <span className="text-xs font-medium absolute ml-48">{short_name}</span>
